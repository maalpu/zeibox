...
func ValidoToken(token string) (bool, error, string) {
	// Dividir el Token (separados por un .)
	...
	if err != nil {
		fmt.Println("Error decoding, No se puede decodificar la parte del token: ", err.Error())
	...
	if err != nil {
		fmt.Println("No se puede decodificar la esturctura JSON ", err.Error())
		return false, err, err.Error()
	}
